...
if [ ! -d "$TARGET_DIR" ]; then
    echo "❌ Erro: Diretório $TARGET_DIR não encontrado. Execute na raiz do projeto (ex: ~/.config/nvim/)."
    exit 1
fi

echo "🚀 Iniciando criação da feature: Smart Auto-Scroll..."

# 1. Cria o módulo Scroller Modular
...
return M
EOF
echo "✅ Criado: $TARGET_DIR/ui/scroller.lua"

# 2. Cria o teste para o Scroller
cat << 'EOF' > "$TARGET_DIR/tests/scroller_spec.lua"
...
EOF
echo "✅ Criado: $TARGET_DIR/tests/scroller_spec.lua"

# 3. Atualiza o init.lua para integrar o Scroller
cat << 'EOF' > "$TARGET_DIR/init.lua"
...
return M
EOF
echo "✅ Substituído: $TARGET_DIR/init.lua com a integração concluída."

echo "🎉 Pronto! O Smart Scroller foi instalado respeitando perfeitamente suas regras."
