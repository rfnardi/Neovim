...
M.gemini = {
	make_request = function(api_config, messages, api_keys, last_sig, callback)
		...
		local partial = ""
		vim.fn.jobstart(cmd, {
			on_e-- Dentro de M.gemini.make_request
			on_stdout = function(_, data)
				if not data then return end
				-- O Gemini stream retorna objetos dentro de um array: [ {...}, {...} ]
				-- Precisamos limpar os colchetes e vírgulas que o curl traz
				local raw = table.concat(data, "")
				local json_str = raw:gsub("^%s*%[%s*", ""):gsub("%s*,?%s*%]%s*$", ""):gsub("^%s*,%s*", "")

				if json_str == "" then return end

				-- Tenta decodar cada linha/objeto
				for line in json_str:gmatch("[^\r\n]+") do
					local ok, decoded = pcall(vim.fn.json_decode, line)
					if ok and decoded.candidates and decoded.candidates[1].content then
						local parts = decoded.candidates[1].content.parts
						if parts and parts[1].text then
							callback(parts[1].text, nil, false)
						end
					elseif ok and decoded.error then
						callback(nil, decoded.error.message or "Erro API", false)
					end
				end
				endxit = function() callback(nil, nil, true) end,
			})
		end,
	}
