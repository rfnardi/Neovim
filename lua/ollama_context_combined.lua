-- ~/.config/nvim/lua/ollama_context_combined.lua
...
local function read_folder_context()
  ...
  table.insert(context_lines, "") -- separa ls e cat
  ...
  for _, fname in ipairs(files) do
    ...
    if vim.fn.isdirectory(full_path) == 0 then
      ...
      table.insert(context_lines, "") -- separa arquivos
    end
  end
...
function M.SendFromPopup()
  if not M.popup_buf or not api.nvim_buf_is_valid(M.popup_buf) then
    vim.notify("Popup não está aberto. Use :Context ou :Context folder", vim.log.levels.WARN)
    return
  end
  ...
  if user_text == "" then
    ...
    return
		...
		-- marca envio (apenas uma vez, direto no final do buffer)
	api.nvim_buf_set_lines(buf, -1, -1, false, { "[mensagem enviada]" })
	vim.notify("mensagem enviada", vim.log.levels.INFO)

	table.insert(M.history, { user = user_text, ai = nil }) -- histórico simplificado
  ...
  vim.fn.jobstart(cmd, {
    ...
    on_stdout = function(_, data, _)
      ...
      ai_content = "## IA >> " .. ai_content
			...
			vim.schedule(function()
				-- pega o índice da última linha aberta
				local last_line = api.nvim_buf_line_count(buf) - 1

				-- insere "[mensagem recebida]" no final
				insert_after(buf, last_line, { "[mensagem recebida]" })

				-- insere conteúdo da IA logo depois
				local ai_lines = split_lines(ai_content)
				api.nvim_buf_set_lines(buf, -1, -1, false, ai_lines)

				-- prepara prompt para próxima entrada
